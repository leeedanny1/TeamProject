...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/user/**", "/chat", "/chatting/**", "/items/sale", "my-shop")
				...
				.deleteCookies("JSESSIONID").and().formLogin() // 로그인 페이지 커스텀
				.loginPage("/sign-in-select") // get
				.usernameParameter("user_id").passwordParameter("user_password").loginProcessingUrl("/sign-in") // post
																												// controller를
																												// 따로
																												// 만들필요
																												// 없음(security가
				...
				.failureUrl("/sign-in").and().oauth2Login().loginPage("/auth/signin")// 로그인할주소
				.defaultSuccessUrl("/sign-up-social")// oauth2로그인이 성공하였을때
				.userInfoEndpoint().userService(principaloauth2userservice);// 서비스
	}
}